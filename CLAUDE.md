...
Tests use Swift Testing (`@Suite` structs, `@Test` methods, `#expect`/`#require`) and are fixture-driven. Each rule test loads a checked-in `.xcodeproj` from `Tests/XCLintTests/TestData/` via `Bundle.module.testDataURL(named:)`, builds an `XCLinter.Environment` directly, invokes the rule struct alone (bypassing `ruleMap` and configuration), and asserts violations are empty or non-empty. Fixtures are usually paired — a passing and a failing project per rule — and are copied as bundle resources by `.copy("TestData")` in `Package.swift`.
